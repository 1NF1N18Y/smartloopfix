...
impl PrivateKey {
    ...
    pub fn generate_halfy(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "halfy_helpers")]
    pub fn generate_halfy_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_halfy(rng))
            .collect()
    }
