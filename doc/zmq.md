...
The Halfy Core daemon can be configured to act as a trusted "border
router", implementing the halfy wire protocol and relay, making
...
The ZeroMQ feature in Halfy Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building halfyd:
...
For instance:
    ...
    $ halfyd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/halfyd.tx.raw \
...
These options can also be provided in halfy.conf.
...
From the perspective of halfyd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into halfyd directly. Furthermore, no information is
...
using. Halfyd appends an up-counting sequence number to each
