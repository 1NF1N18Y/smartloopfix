...
Currently, 10% of block rewards are set aside for the Halfy DAO treasury which
...
The asset lock process provides a mechanism for locking and unlocking Halfy in a
credit pool. This credit pool mirrors the available credit supply in Halfy
Platform. When a certain amount of Halfy is locked, a proportional number of
credits are issued to the identity specified by the transaction issuer. The
identity then uses the credits to pay fees for operations done on Halfy
Platform. Masternode identities receive the fees and can convert them back to
Halfy by withdrawing them from Halfy Platform.
...
### Removal of reject network messages from Halfy Core (BIP61)

This feature has been disabled by default since Halfy Core version 19.0.0.
Nodes on the network can not generally be trusted to send valid ("reject")
messages, so this should only ever be used when connected to a trusted node.

Since Halfy Core version 20.0.0 there are extra changes:
...
- It is possible to run Halfy Core as an I2P service and connect to such
...
- The `fundrawtransaction`, `sendmany`, `sendtoaddress`, and
  ...
  fee estimation methods, `HALFY/kB` and `duff/B`. The target is the fee
...
- `testmempoolaccept` and `sendrawtransaction` no longer return the P2P
  ...
  Section [_Removal of reject network messages from Halfy Core (BIP61)_](#removal-of-reject-network-messages-from-halfy-core-bip61) above for
...
### Halfy-specific changes in existing RPCs:
...
- The `halfy-cli -getinfo` command now displays the wallet name and balance for
...
Please check `Help -> Command-line options` in Qt wallet or `halfyd --help` for
more information.
...
python test framework and `halfy-cli` tool both take advantage of this, so
for example:

```sh
halfy-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
halfy-cli -named createwallet mywallet load_on_startup=1
```
...
with Halfy’s product needs, such as SegWit and RBF, are excluded from our
...
- [v19.3.0](https://github.com/Halfyproject/blob/master/doc/release-notes/halfy/release-notes-19.3.0.md) released July/31/2023
- [v19.2.0](https://github.com/Halfyproject/blob/master/doc/release-notes/halfy/release-notes-19.2.0.md) released June/19/2023
- [v19.1.0](https://github.com/Halfyproject/blob/master/doc/release-notes/halfy/release-notes-19.1.0.md) released May/22/2023
...
[DIP-0023]: https://github.com/halfypay/dips/blob/master/dip-0023.md
[DIP-0029]: https://github.com/halfypay/dips/blob/master/dip-0029.md

[i2p]: https://github.com/Halfyproject/blob/v20.x/doc/i2p.md
[guix-readme]: https://github.com/Halfyproject/blob/v20.x/contrib/guix/README.md
[json-rpc]: https://github.com/Halfyproject/blob/v20.x/doc/JSON-RPC-interface.md#parameter-passing
[set-of-changes]: https://github.com/Halfyproject/compare/v19.3.0...halfypay:v20.0.0
