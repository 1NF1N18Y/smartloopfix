...
TMPDIR_PREFIX = "halfy_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave halfyds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop halfyds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use halfy-cli instead of RPC for all commands")
        parser.add_argument("--halfyd-arg", dest="halfyd_extra_args", default=[], action="append",
                            help="Pass extra args to all halfyd instances")
    ...
    def setup(self):
        ...
        fname_bitcoind = os.path.join(
            ...
            "src",
            "halfyd" + config["environment"]["EXEEXT"]
        )
        fname_bitcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "halfy-cli" + config["environment"]["EXEEXT"]
        )
        self.options.bitcoind = os.getenv("BITCOIND", default=fname_bitcoind)
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=fname_bitcoincli)

        self.extra_args_from_options = self.options.halfyd_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: halfyds were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'halfyd', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'halfy-cli', self.options.bitcoincli) for v in versions]
    ...
    def dynamically_initialize_datadir(self, node_p2p_port, node_rpc_port):
        ...
        with open(os.path.join(new_data_dir, "halfy.conf"), 'w', encoding='utf8') as f:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as halfyd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in halfy.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if halfyd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("halfyd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if halfy-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("halfy-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if halfy-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("halfy-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether halfy-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether halfy-wallet was compiled."""
...
class HalfyTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_halfy_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False, evo_count=0):
        ...
        self.set_halfy_dip8_activation(200)
        ...
        # This is nRequestTimeout in halfy-q-recovery thread
    ...
    def set_halfy_dip8_activation(self, activate_after_block):
    ...
    def set_halfy_llmq_test_params(self, llmq_size, llmq_threshold):
